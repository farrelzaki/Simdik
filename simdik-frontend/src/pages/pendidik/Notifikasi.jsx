  ...
  const fetchNotifikasi = () => {
    ...
    api.get('/pendidik/notifikasi')
      .then(res => {
        ...
        window.dispatchEvent(new CustomEvent('updateNotifPendidik', { detail: res.data.belum_dibaca || 0 }))
      })
      .catch(() => { })
  ...
  const tandaiSemuaDibaca = async () => {
    ...
    try {
      ...
      setNotifikasi(prev => {
        ...
        window.dispatchEvent(new CustomEvent('updateNotifPendidik', { detail: 0 }))
        return next
      })
    } catch (error) {
      console.error(error)
  ...
  const tandaiDibaca = async (id) => {
    try {
      ...
      setNotifikasi(prev => {
        ...
        window.dispatchEvent(new CustomEvent('updateNotifPendidik', { detail: unreadCount }))
        return next
      })
    } catch (error) {
      console.error(error)
          ...
          <div className="relative">
            <button 
              ...
              className={`flex items-center gap-2 px-4 py-2 border rounded-xl text-sm transition-colors ${
                filterMode !== 'semua' 
                ? 'border-[#054a5c] text-[#054a5c] bg-[#054a5c]/5' 
                ...
                {['semua', 'belum_dibaca', 'sudah_dibaca'].map(mode => (
                  <button 
                    ...
                    className={`w-full text-left px-3 py-2 rounded-lg text-sm transition-colors ${
                      filterMode === mode ? 'bg-[#054a5c]/10 text-[#054a5c] font-medium' : 'text-gray-600 hover:bg-gray-50'
                    }`}>
