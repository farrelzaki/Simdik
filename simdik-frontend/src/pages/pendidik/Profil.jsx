...
import { Camera, Trash2, GraduationCap, Award, Clock, CheckCircle, XCircle, Send } from 'lucide-react'
...
export default function PendidikProfil() {
  ...
  const fetchData = () => {
    Promise.all([
      ...
      setProfil(p)
      setPerubahan(perubahanRes.data.data || [])
      setForm({
        nama: p.nama || '', no_hp: p.no_hp || '',
        alamat: p.alamat || '', pendidikan_terakhir: p.pendidikan_terakhir || '',
        jabatan: p.jabatan || '', unit_kerja: p.unit_kerja || '',
  ...
  if (loading) return <div className="flex items-center justify-center h-64 text-gray-400 text-sm">Memuat profil...</div>
      ...
      <div className="bg-white rounded-2xl shadow-sm overflow-hidden">
        ...
        <div className="px-6 pb-6">
          <div className="flex items-end justify-between -mt-12 mb-4">
            {/* Avatar */}
            <div className="relative" onMouseLeave={() => setShowFotoMenu(false)}>
              <div
                className="w-24 h-24 rounded-2xl border-4 border-white shadow-md overflow-hidden bg-[#054a5c] flex items-center justify-center cursor-pointer relative"
                onMouseEnter={() => setShowFotoMenu(true)}
              >
                {profil?.foto_url ? (
              ...
              {/* Foto Menu */}
              {showFotoMenu && (
                <div className="absolute top-full left-0 mt-1 bg-white rounded-xl shadow-lg border border-gray-100 z-20 w-36 overflow-hidden">
                  <label className="flex items-center gap-2 px-3 py-2.5 text-sm text-gray-700 hover:bg-gray-50 cursor-pointer">
                    <Camera size={14} /> Upload Foto
                    <input type="file" className="hidden" accept="image/*" ref={fotoRef} onChange={handleUploadFoto} />
                  </label>
                  {profil?.foto_profil && (
                    <button onClick={handleHapusFoto}
                      className="flex items-center gap-2 px-3 py-2.5 text-sm text-red-500 hover:bg-red-50 w-full">
          ...
          <p className="text-sm text-gray-500">{profil?.jabatan || 'Pendidik'} {profil?.unit_kerja ? `• ${profil.unit_kerja}` : ''}</p>
        ...
        <div className="col-span-2 space-y-4">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            <div className="flex items-center gap-2 mb-4">
              ...
              {profil?.updated_at && (
                <span className="ml-auto text-xs text-[#054a5c] font-medium">
            ...
            <div className="grid grid-cols-2 gap-4">
              {[
                ...
                { label: 'Tempat, Tgl Lahir',    value: profil?.tempat_lahir ? `${profil.tempat_lahir}, ${profil.tanggal_lahir ? new Date(profil.tanggal_lahir).toLocaleDateString('id-ID') : ''}` : '—' },
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            {profil?.pendidikan_terakhir ? (
              ...
              <p className="text-sm text-gray-400">Belum ada data riwayat pendidikan</p>
            )}
          </div>
          ...
          <div className="bg-white rounded-2xl shadow-sm p-5">
            ...
            {profil?.dokumen?.data_sertifikasi ? (
              <div className="p-3 bg-gray-50 rounded-xl flex items-center justify-between">
                <p className="text-sm font-medium text-gray-700">Sertifikasi tersedia</p>
                <a href={`http://localhost:8000/storage/${profil.dokumen.data_sertifikasi}`}
                  target="_blank" rel="noreferrer"
                  className="text-xs text-[#054a5c] font-medium hover:underline">
                  Lihat →
                </a>
              </div>
            ) : (
          ...
          {showForm && (
            <div className="bg-white rounded-2xl shadow-sm p-5">
              <div className="flex items-center justify-between mb-4">
                <h3 className="font-bold text-[#054a5c] text-sm">Ajukan Perubahan</h3>
                <button onClick={() => setShowForm(false)} className="text-gray-400 hover:text-gray-600 text-xs">Batal</button>
              </div>
              <form onSubmit={handleAjukan} className="space-y-3">
                ...
                <button type="submit" disabled={saving}
                  className="w-full flex items-center justify-center gap-2 bg-[#054a5c] text-white py-2.5 rounded-xl text-sm font-medium hover:bg-[#033a47] disabled:opacity-60">
                  ...
                  return (
                    <div key={p.id_perubahan} className="border border-gray-100 rounded-xl p-3">
                      <div className="flex items-center justify-between mb-2">
                        <span className="text-xs font-medium text-gray-600 capitalize">{p.tipe}</span>
                      ...
                      <div className="space-y-1">
                        {Object.entries(p.data_baru || {}).map(([k, v]) => (
                          <div key={k} className="text-xs text-gray-500">
                            <span className="font-medium">{FIELD_LABELS[k] || k}:</span> {v}
                          </div>
                        ))}
                      </div>
                      {p.catatan && (
